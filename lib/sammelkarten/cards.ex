defmodule Sammelkarten.Cards do
  ...
  def create_card(attrs \\ %{}) do
    ...
    transaction_result = :mnesia.transaction(fn ->
      :mnesia.write({:cards, card.id, card.name, card.image_path, card.current_price, 
                     card.price_change_24h, card.price_change_percentage, card.rarity,
                     card.description, card.last_updated})
    end)
  ...
  def get_card(id) do
    case :mnesia.transaction(fn -> :mnesia.read({:cards, id}) end) do
      {:atomic, []} -> 
        {:error, :not_found}
      {:atomic, [{:cards, id, name, image_path, current_price, price_change_24h, 
  ...
  def list_cards do
    case :mnesia.transaction(fn -> :mnesia.select(:cards, [{:_, [], [:"$_"]}]) end) do
      {:atomic, records} -> 
        cards = Enum.map(records, fn {_table, id, name, image_path, current_price, 
                                      price_change_24h, price_change_percentage, rarity, 
  ...
  def update_card_price(card_id, new_price) do
    with {:ok, card} <- get_card(card_id) do
      ...
      price_change_percentage = if card.current_price > 0 do
      ...
      transaction_result = :mnesia.transaction(fn ->
        # Update card
        :mnesia.write({:cards, updated_card.id, updated_card.name, updated_card.image_path, 
                       updated_card.current_price, updated_card.price_change_24h, 
                       updated_card.price_change_percentage, updated_card.rarity,
                       updated_card.description, updated_card.last_updated})
        
        # Create price history entry
        price_history = PriceHistory.new(%{
          card_id: card_id,
          price: new_price,
          volume: :rand.uniform(1000)  # Simulated volume
        })
        
        :mnesia.write({:price_history, price_history.id, price_history.card_id, 
                       price_history.price, price_history.timestamp, price_history.volume})
      end)
