defmodule Sammelkarten.MarketInsights do
  ...
  def get_trending_analysis(days_back \\ 7) do
    with {:ok, recent_trades} <- get_all_user_trades(days_back),
      ...
      older_trades = Enum.filter(comparison_trades, fn trade ->
  ...
  defp analyze_card_popularity(trades) do
    card_stats = 
      ...
      |> Enum.map(fn {card_id, card_trades} ->
        total_volume = Enum.reduce(card_trades, 0, fn trade, acc ->
          acc + (trade.price * trade.quantity)
        end)
    ...
    %{
      by_volume: card_stats |> Enum.sort_by(fn {_, stats} -> stats.total_volume end, :desc) |> Enum.take(10),
      by_trades: card_stats |> Enum.sort_by(fn {_, stats} -> stats.trade_count end, :desc) |> Enum.take(10),
      by_traders: card_stats |> Enum.sort_by(fn {_, stats} -> stats.unique_traders end, :desc) |> Enum.take(10)
    }
  end
  ...
  defp analyze_market_sentiment(trades) do
    ...
    sentiment_score = if total_volume > 0 do
  ...
  defp get_trending_cards(_recent, _older), do: ["BITCOIN_HOTEL", "BLOCKTRAINER", "CHRISTIAN_DECKER"]
  defp identify_emerging_traders(_recent, _older), do: ["new_trader_1", "new_trader_2"]
  defp analyze_sentiment_shift(_recent, _older), do: %{from: :neutral, to: :bullish, strength: 0.3}
  defp analyze_price_momentum(_trades), do: %{direction: :upward, strength: 0.25}
  defp detect_market_anomalies(_trades), do: []
  defp count_trading_pairs(trades), do: length(Enum.uniq_by(trades, fn t -> {t.user_pubkey, t.card_id} end))
  defp calculate_network_density(_trades), do: 0.15
  defp identify_central_traders(_trades), do: ["central_trader_1", "central_trader_2"]
  defp identify_trading_clusters(_trades), do: [%{size: 5, activity: :high}, %{size: 8, activity: :medium}]
  defp calculate_influence_metrics(_trades), do: %{avg_influence: 0.3, max_influence: 0.8}
  defp analyze_collaboration_patterns(_trades), do: %{cooperation_index: 0.65, network_effects: :positive}
  ...
  defp identify_temporal_trends(_trades), do: %{weekly_pattern: :consistent, growth_trend: :positive}
  defp create_user_profile(trades), do: %{trade_count: length(trades), volume: calculate_total_volume(trades), behavior: :active}
  ...
  defp calculate_card_correlations(_combinations), do: %{"BITCOIN_HOTEL" => %{"BLOCKTRAINER" => 0.75}}
